...
VARIANT="${K7GPS_VARIANT:-dashboard}"
...
for f in K7GPSViewController.m K7GPSStore.m K7GPSEngine.m K7GPSBootstrap.m; do
  test -s "$SRC_DIR/$f" || { echo "Missing $f"; exit 1; }
done
...
IFS=',' read -r -a BUNDLES <<< "$TARGET_IDS"
FILTER=""
for b in "${BUNDLES[@]}"; do FILTER+="\"$b\","; done
FILTER="${FILTER%,}"
...
make_deb() {
  ...
  prefix="$root"
  [ "$mode" = "rootless" ] && prefix="$root/var/jb"
  mkdir -p "$prefix/Library/MobileSubstrate/DynamicLibraries"
  cp "$PROJECT_DIR/K7GPS.dylib" "$prefix/Library/MobileSubstrate/DynamicLibraries/K7GPS.dylib"
  cat > "$prefix/Library/MobileSubstrate/DynamicLibraries/K7GPS.plist" <<EOF
...
Description: K7GPS source build - $SAFE_VARIANT
Maintainer: WFX
Author: WFX
Section: Tweaks
EOF
  chmod 0755 "$root/DEBIAN"
  chmod 0644 "$root/DEBIAN/control" "$prefix/Library/MobileSubstrate/DynamicLibraries/K7GPS.plist" "$prefix/Library/MobileSubstrate/DynamicLibraries/K7GPS.dylib"
  out="$PROJECT_DIR/${PRODUCT}_v${VERSION}_${mode}.deb"
  dpkg-deb --root-owner-group -Zgzip --build "$root" "$out"
  echo "$out"
}
